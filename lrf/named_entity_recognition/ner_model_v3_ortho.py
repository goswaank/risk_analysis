...
def plot_graph(plot_typ="ggplot", fig_size = (12,12), fields = ["acc","val_acc"]):
    ...
    for field in fields:
        ...
        plt.plot(hist[field])
    plt.title('ORTHO model accuracy and loss')
...
def get_char_embedding_model():
    ...
    ortho_emb_word = Embedding(input_dim=n_ortho_words + 2, output_dim=20, input_length=constants.MAX_SENT_LEN, mask_zero=True)(
        ortho_word_in)
    ...
    ortho_char_in = Input(shape=(constants.MAX_SENT_LEN, constants.MAX_WORD_LEN,))
    ortho_emb_char = TimeDistributed(
        Embedding(input_dim=n_ortho_chars + 2, output_dim=10, input_length=constants.MAX_WORD_LEN, mask_zero=True))(
        ortho_char_in)
    ...
    ortho_char_encoding = TimeDistributed(LSTM(units=20, return_sequences=False, recurrent_dropout=0.5))(ortho_emb_char)
    print(char_encoding.shape, ' | ',ortho_char_encoding.shape)

    ## main LSTM
    x = concatenate([char_encoding,emb_word,ortho_char_encoding,ortho_emb_word])
...
if __name__=='__main__':
    ...
    extras = ['PAD', 'UNK']
    ortho_char2ind, ind2char_ortho = prepare_tokens(ortho_chars, extras)
    ...
    ## Ortho
    X_ortho_char_train = get_char_sequences(train_tweets, ortho_char2ind,isortho=True)
    X_ortho_char_validation = get_char_sequences(validation_tweets, ortho_char2ind,isortho=True)
    ...
    ner_model.summary()
    print('len(validation) : ',len(y_validation))

    ## Fitting the model
    history = ner_model.fit([np.array(X_char_train).reshape((len(X_char_train), max_sent_len, max_word_len)), X_word_train, np.array(X_ortho_char_train).reshape((len(X_ortho_char_train), max_sent_len, max_word_len)), X_ortho_word_train],
                            np.array(y_train).reshape(len(y_train),max_sent_len, 1),
                            batch_size=constants.BATCH_SIZE, epochs = constants.EPOCHS, validation_data=([np.array(X_char_validation).reshape((len(X_char_validation), max_sent_len, max_word_len)), X_word_validation ,
                                                                                                          np.array(X_ortho_char_validation).reshape((len(X_ortho_char_validation), max_sent_len, max_word_len)), X_ortho_word_validation],
    ...
    y_pred = ner_model.predict([np.array(X_char_validation).reshape((len(X_char_validation),max_sent_len, max_word_len)), X_word_validation, np.array(X_ortho_char_validation).reshape((len(X_ortho_char_validation),max_sent_len, max_word_len)), X_ortho_word_validation])
    ...
    with open('tmp_ortho.pkl','wb') as fp:
